export interface WebPromptState {
  /** Whether web tools (web_search / fetch_url) are in this run's tool list. */
  toolsAvailable: boolean;
  /** Whether the reader-proxy fallback for fetch_url is enabled in Settings. */
  readerFallback: boolean;
}

const NO_WEB_ACCESS: WebPromptState = { toolsAvailable: false, readerFallback: false };

export function buildSystemPrompt(workbookId: string, web: WebPromptState = NO_WEB_ACCESS): string {
  const rules = [
    ...
    ...(web.toolsAvailable ? webRules(web.readerFallback) : [NO_WEB_RULE]),
...
const NO_WEB_RULE =
  '**No web access.** Web tools (`web_search`, `fetch_url`) are NOT available in this session. Never attempt to call them. If the task needs external data, say so and tell the user they can configure web access in Settings → Web Access.';

function webRules(readerFallback: boolean): string[] {
  return [
