...
export const WEB_SEARCH: ToolSpec = {
  name: 'web_search',
  description: 'Search the web for current information. Returns result links and snippets by default. Set include_content to true to also get extracted page text per result (supported by some providers; others return snippets only) - prefer this over fetch_url when a site is likely to block cross-origin requests, since fetch_url fails on CORS-blocked pages. If results suggest several distinct datasets or interpretations of the user request, use request_user_choice before fetching large data. When the query routes to the keyless catalogues (the default backend), set source to pick the catalogue that best matches the question.',
  parameters: {
    type: 'object',
    properties: {
      ...
      source: {
        ...
        description: `Keyless catalogue to search; honoured when the query routes to the keyless bundle (the default backend), ignored when a keyed provider handles it. Sources: ${SOURCE_GUIDE}. Default 'auto' searches the general sources (wikipedia, wikidata, worldbank); pick a specific source for statistical, regional, or weather questions.`,
      },
    },
    required: ['query'],
