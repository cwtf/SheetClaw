...
- Assistant narration attached to tool calls is grouped into a default-collapsed **Thought process** card next to the tool-call chain. Expanding it shows each intermediate narration step; the final assistant answer remains visible. Both collapsible cards are non-shrinking flex items so transcript overflow scrolls instead of compressing their headers.
