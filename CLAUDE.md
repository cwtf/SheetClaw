...
Optional. [src/web/](src/web/) contains `fetch.ts` (fetch_url tool with Jina reader proxy fallback) and `search.ts` (web_search routing to Tavily / SearXNG / Google CSE / Jina / Wikipedia). Provider selection and BYOK keys are configured in Settings → Web Access.
